// Archivo: js-vivi/consumidor-bebidas.js

document.addEventListener('DOMContentLoaded', () => {
    const productGrid = document.getElementById('product-grid-container');

    async function cargarProductosBebidas() {
        if (!productGrid) return;
        productGrid.innerHTML = '<p>Cargando productos...</p>';

        try {
            const url = 'http://localhost:3000/api/productos?tipo_producto=bebidas&categoria=comida-por-caducarse';
            console.log('[BEBIDAS] Fetch =>', url);
            const respuesta = await fetch(url);
            
            if (!respuesta.ok) throw new Error('Error al cargar los productos');
            const productos = await respuesta.json();
            console.log('[BEBIDAS] Total backend =>', productos.length);
            const filtrados = productos.filter(p => p.categoria === 'comida-por-caducarse' && p.tipo_producto === 'bebidas');
            console.log('[BEBIDAS] Total tras filtro cliente =>', filtrados.length);
